...
async function _syncBSE500FromKite(preferredSymbol) {
  ...
  if (existingRow?.value) { try { existing = JSON.parse(existingRow.value); } catch (e) {} }
  const todayInt = (() => { const t = new Date(); return t.getFullYear() * 10000 + (t.getMonth() + 1) * 100 + t.getDate(); })();
  // Drop weekend AND future-dated rows from whatever's already stored — a bad
  // manual upload (typo'd year, swapped day/month, stray forward-dated row)
  // must not be trusted as "the last known date" or it inverts the from/to
  // range below and Kite silently returns nothing.
  ...
  const hist = await _kiteGet(`/instruments/historical/${chosen.token}/day?from=${fromDate}&to=${toDate}&continuous=0&oi=0`);
  ...
  return { ok: true, symbol: chosen.tradingsymbol, rows: dataOut.length, added: fresh.length, label };
}
