...
// ── BSE instrument map — same pattern as the NSE map above, used to
// resolve the BSE 500 index's instrument_token without hardcoding a
// number we can't verify from outside a live Kite session. ──
let _bseInstrumentMap = {}; // tradingsymbol -> {token, name, segment}
let _bseInstrumentMapDate = null;
async function _ensureBSEInstrumentMap() {
  const today = new Date().toISOString().split('T')[0];
  if (_bseInstrumentMapDate === today && Object.keys(_bseInstrumentMap).length) return;
  if (!_kiteToken) return;
  try {
    const resp = await fetch(`${KITE_BASE}/instruments/BSE`, { headers: { 'Authorization': `token ${KITE_API_KEY}:${_kiteToken}`, 'X-Kite-Version': '3' } });
    const csv = await resp.text();
    const lines = csv.split('\n');
    const header = lines[0].split(',');
    const tokenIdx = header.indexOf('instrument_token');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const sym = cols[symIdx];
      if (!sym) continue;
      map[sym] = { token: cols[tokenIdx], name: (cols[nameIdx] || '').replace(/^"|"$/g, ''), segment: cols[segIdx] || '' };
    }
    if (Object.keys(map).length) { _bseInstrumentMap = map; _bseInstrumentMapDate = today; console.log('[Kite] BSE instrument map loaded:', Object.keys(map).length, 'symbols'); }
  } catch (e) { console.warn('[Kite] BSE instrument map fetch failed:', e.message); }
}

function _findBSE500Candidates() {
  return Object.entries(_bseInstrumentMap)
    .filter(([sym, info]) => info.segment === 'INDICES' && /500/.test(sym + ' ' + info.name))
    .map(([sym, info]) => ({ tradingsymbol: sym, name: info.name, token: info.token }));
}

function _isWeekendDateInt(di) {
  const y = Math.floor(di / 10000), m = Math.floor((di % 10000) / 100) - 1, d = di % 100;
  const dow = new Date(y, m, d).getDay();
  return dow === 0 || dow === 6;
}

// ── BSE 500 auto-sync from Kite — replaces the manual excel upload for
// the Alpha Tracker benchmark. Resolves the index token once (cached
// daily), pulls daily historical closes, filters to real trading days,
// and writes into the SAME app_settings 'bse500' key the manual upload
// path already uses (see /api/meta/bse500 + frontend saveMeta('bse500',...))
// — so the frontend needs zero changes to consume it. ──
async function _syncBSE500FromKite(preferredSymbol) {
  await _ensureBSEInstrumentMap();
  const candidates = _findBSE500Candidates();
  let chosen = null;
  if (!preferredSymbol) {
    try {
      const { data: pinned } = await _supabase.from('app_settings').select('value').eq('key', 'bse500_kite_symbol').single();
      if (pinned?.value) preferredSymbol = pinned.value;
    } catch (e) {}
  }
  if (preferredSymbol && _bseInstrumentMap[preferredSymbol]) {
    chosen = { tradingsymbol: preferredSymbol, ..._bseInstrumentMap[preferredSymbol] };
  } else {
    chosen = candidates.find(c => c.tradingsymbol.replace(/\s/g, '').toUpperCase() === 'BSE500')
      || (candidates.length === 1 ? candidates[0] : null);
  }
  if (!chosen) return { ok: false, error: 'ambiguous_or_not_found', candidates };

  // Merge forward from whatever we already have, small overlap for safety.
  const { data: existingRow } = await _supabase.from('app_settings').select('value').eq('key', 'bse500').single();
  let existing = { data: [], label: '' };
  if (existingRow?.value) {
    try {
      // app_settings.value may come back as a JSON string (text column) or
      // already-parsed object (jsonb column) depending on schema — handle both,
      // don't let JSON.parse(anObject) silently wipe out real existing data.
      existing = typeof existingRow.value === 'string' ? JSON.parse(existingRow.value) : existingRow.value;
    } catch (e) { console.warn('[BSE500 sync] failed to parse existing app_settings.bse500:', e.message); }
  }
  const todayInt = (() => { const t = new Date(); return t.getFullYear() * 10000 + (t.getMonth() + 1) * 100 + t.getDate(); })();
  const existingData = (Array.isArray(existing.data) ? existing.data : [])
    .filter(row => !_isWeekendDateInt(row[0]) && row[0] <= todayInt);

  const fromDate = existingData.length ? (() => {
    const last = existingData[existingData.length - 1][0];
    const y = Math.floor(last / 10000), m = Math.floor((last % 10000) / 100) - 1, d = last % 100;
    const dt = new Date(y, m, d); dt.setDate(dt.getDate() - 5);
    return dt.toISOString().split('T')[0];
  })() : '2015-01-01';
  const toDate = new Date().toISOString().split('T')[0];

  // Kite's daily-candle historical endpoint rejects/empties out on overly
  // wide ranges (esp. on a first-ever sync going back to 2015) — chunk into
  // ~1-year windows like kiteHistoricalService.js already does for Nifty,
  // and surface any Kite-side error instead of silently returning 0 rows.
  const candles = [];
  let kiteErrors = [];
  ...
  while (cur <= endDate) {
    ...
    try {
      const hist = await _kiteGet(`/instruments/historical/${chosen.token}/day?from=${from}&to=${to}&continuous=0&oi=0`);
      if (hist?.status === 'error') { kiteErrors.push(`${from}–${to}: ${hist.message || hist.error_type || 'unknown Kite error'}`); }
      else if (Array.isArray(hist?.data?.candles)) { candles.push(...hist.data.candles); }
    } catch (e) { kiteErrors.push(`${from}–${to}: ${e.message}`); }
    await new Promise(r => setTimeout(r, 200));
    cur = new Date(chunkEnd); cur.setDate(cur.getDate() + 1);
  }
  if (kiteErrors.length) console.warn('[BSE500 sync] Kite errors:', kiteErrors.join(' | '));

  const fresh = candles.map(c => {
    const [y, mo, d] = c[0].split('T')[0].split('-').map(Number);
    return [y * 10000 + mo * 100 + d, c[4]];
  }).filter(row => !_isWeekendDateInt(row[0]));

  const merged = {};
  existingData.forEach(([di, px]) => { merged[di] = px; });
  fresh.forEach(([di, px]) => { merged[di] = px; });
  const dataOut = Object.entries(merged).map(([di, px]) => [parseInt(di), px]).sort((a, b) => a[0] - b[0]);

  // Flag implausible single-day swings in Kite's own returned data (a broad
  // 500-stock index moving >4% in one day is rare) — surfaced in the response
  // and logged, not silently trusted. Mirrors the check already run on manual
  // Excel uploads (findBenchOutliers in the frontend), now applied here too
  // since Kite sync is now the primary path.
  ...
  for (let i = 1; i < dataOut.length; i++) {
    const prev = dataOut[i - 1][1], cur = dataOut[i][1];
    if (!prev) continue;
    const chg = (cur - prev) / prev * 100;
    if (Math.abs(chg) > 4) outliers.push({ date: fmtInt(dataOut[i][0]), prevDate: fmtInt(dataOut[i - 1][0]), chg: Math.round(chg * 100) / 100 });
  }
  if (outliers.length) console.warn('[BSE500 sync] Suspicious single-day moves from Kite:', JSON.stringify(outliers));

  const label = dataOut.length ? `${dataOut.length} days (${fmtInt(dataOut[0][0])} – ${fmtInt(dataOut[dataOut.length - 1][0])}) · via Kite (${chosen.tradingsymbol})` : '';

  const valueJson = JSON.stringify({ data: dataOut, label });
  const { data: exists } = await _supabase.from('app_settings').select('key').eq('key', 'bse500').single();
  if (exists) await _supabase.from('app_settings').update({ value: valueJson, updated_at: new Date().toISOString() }).eq('key', 'bse500');
  else await _supabase.from('app_settings').insert({ key: 'bse500', value: valueJson, updated_at: new Date().toISOString() });

  return { ok: true, symbol: chosen.tradingsymbol, rows: dataOut.length, added: fresh.length, label, kiteErrors: kiteErrors.length ? kiteErrors : undefined, outliers: outliers.length ? outliers : undefined };
}

app.get('/api/kite/bse500-candidates', requireAuth, async (req, res) => {
  try { await _ensureBSEInstrumentMap(); res.json({ candidates: _findBSE500Candidates() }); }
  catch (err) { res.status(500).json({ error: safeError(err, 'bse500_candidates') }); }
});

// Diagnostic — returns the raw Kite candles (and any Kite-side error) for a
// small date range, so a "why is this day missing" question can be answered
// by looking at what Kite actually sent back instead of guessing.
app.get('/api/kite/bse500-debug', requireAuth, requireRole('admin'), async (req, res) => {
  try {
    await _ensureBSEInstrumentMap();
    const candidates = _findBSE500Candidates();
    const symbol = req.query.symbol || candidates.find(c => c.tradingsymbol.replace(/\s/g, '').toUpperCase() === 'BSE500')?.tradingsymbol || candidates[0]?.tradingsymbol;
    const inst = symbol && _bseInstrumentMap[symbol];
    if (!inst) return res.status(404).json({ error: 'symbol not resolved', candidates });
    const from = req.query.from || '2026-08-01';
    const to = req.query.to || '2026-08-17';
    const hist = await _kiteGet(`/instruments/historical/${inst.token}/day?from=${from}&to=${to}&continuous=0&oi=0`);
    res.json({ symbol, token: inst.token, from, to, raw: hist });
  } catch (err) { res.status(500).json({ error: safeError(err, 'bse500_debug') }); }
});

app.get('/api/kite/bse500-symbol', requireAuth, async (req, res) => {
  try {
    const { data } = await _supabase.from('app_settings').select('value').eq('key', 'bse500_kite_symbol').single();
    res.json({ symbol: data?.value || null });
  } catch (err) { res.status(500).json({ error: safeError(err, 'bse500_symbol_get') }); }
});

app.post('/api/kite/bse500-symbol', requireAuth, requireRole('admin'), async (req, res) => {
  try {
    const { symbol } = req.body || {};
    if (!symbol) return res.status(400).json({ error: 'symbol required' });
    await _ensureBSEInstrumentMap();
    if (!_bseInstrumentMap[symbol]) return res.status(404).json({ error: 'symbol not found on Kite BSE instrument list' });
    const { data: exists } = await _supabase.from('app_settings').select('key').eq('key', 'bse500_kite_symbol').single();
    if (exists) await _supabase.from('app_settings').update({ value: symbol, updated_at: new Date().toISOString() }).eq('key', 'bse500_kite_symbol');
    else await _supabase.from('app_settings').insert({ key: 'bse500_kite_symbol', value: symbol, updated_at: new Date().toISOString() });
    audit(req.user.id, 'BSE500_SYMBOL_PIN', { symbol });
    res.json({ ok: true, symbol });
  } catch (err) { res.status(500).json({ error: safeError(err, 'bse500_symbol_set') }); }
});

app.post('/api/kite/sync-bse500', requireAuth, requireRole('admin'), async (req, res) => {
  try {
    const result = await _syncBSE500FromKite(req.body?.symbol);
    if (!result.ok) return res.status(409).json(result);
    audit(req.user.id, 'BSE500_KITE_SYNC', { symbol: result.symbol, rows: result.rows });
    // Auto-pin the resolved symbol so future syncs (incl. the daily cron) skip resolution ambiguity entirely.
    try {
      const { data: exists } = await _supabase.from('app_settings').select('key').eq('key', 'bse500_kite_symbol').single();
      if (exists) await _supabase.from('app_settings').update({ value: result.symbol, updated_at: new Date().toISOString() }).eq('key', 'bse500_kite_symbol');
      else await _supabase.from('app_settings').insert({ key: 'bse500_kite_symbol', value: result.symbol, updated_at: new Date().toISOString() });
    } catch (e) {}
    res.json(result);
  } catch (err) { res.status(500).json({ error: safeError(err, 'bse500_sync') }); }
});
...
cron.schedule('0 16 * * 1-5', () => { _syncBSE500FromKite().catch(e => console.error('[BSE500 Kite cron]', e.message)); }, { timezone: 'Asia/Kolkata' }); // 4pm IST weekdays, after market close settles
